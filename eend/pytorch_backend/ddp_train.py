...
def train(args):
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for step, (y, t) in tqdm(enumerate(train_iter), ncols=100, total=len(train_iter)):
            y = [yi.to(device) for yi in y] # B x Ti x 23, Ti不一定都=2000
            t = [ti.to(device) for ti in t]
            
            # 先做padding变成tensor，这样nn.DataParallel才能分发数据
            # 但有可能导致每个卡上的padding过多，因此在model内部再做一次切分（把ilens_tensor传入），避免过多无用计算；
            ...
            output = model(y, ilens_tensor) # model里面会对Ti维度做padding
