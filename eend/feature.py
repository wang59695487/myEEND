...
def get_labeledSTFT(
    ...
    """
    data, rate = kaldi_obj.load_wav(
            rec, start * frame_shift, end * frame_shift) # 因为传进来的start, end是考虑了frame_shift的，所以要提取原始的，得*回去
    Y = stft(data, frame_size, frame_shift) # 这样得到的Y才能最后得到2000帧序列
    filtered_segments = kaldi_obj.segments[rec] # 把这段音频的分割结果提出来
    # TODO: 这里有个思考：
    #                    取的是整段的对话，会不会可能目标chunck中仅包含了部分speaker？
    #                    2000帧对应每帧100ms，即200s，感觉应该不会有，既然是对话，一个人连续说200s不太可能吧
    # filtered_segments = kaldi_obj.segments[kaldi_obj.segments['rec'] == rec]
    # utt2spk中的信息因该是这样的吧:
    # {utt: speaker}
    # {1-1001-1: 1, 1-1001-2: 2....}
    speakers = np.unique(
            [kaldi_obj.utt2spk[seg['utt']] for seg
                in filtered_segments]).tolist() #这句话中speakers的数量
    if n_speakers is None:
        n_speakers = len(speakers)
    T = np.zeros((Y.shape[0], n_speakers), dtype=np.int32)

    if use_speaker_id: # False
        all_speakers = sorted(kaldi_obj.spk2utt.keys())
        S = np.zeros((Y.shape[0], len(all_speakers)), dtype=np.int32)

    for seg in filtered_segments: # 把音频中每一帧打上对应说话人的标签
        speaker_index = speakers.index(kaldi_obj.utt2spk[seg['utt']])
        if use_speaker_id:
